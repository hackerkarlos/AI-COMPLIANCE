...
import { CookiePolicyGenerator, CookieBannerGuidance } from '@/components/cookie-toolkit';
import type { ChecklistStatus } from '@/components/regulations';

export const dynamic = 'force-dynamic';

// ─── Types (mirror of [id]/page.tsx) ─────────────────────────
...
// ─── Applicability reason builder (same as [id]/page.tsx) ────
...
function buildApplicabilityReason(
  ...
  if (criteria.requires_personal_data_processing === true && company.processes_personal_data) {
...
export default async function EPrivacyDetailPage() {
  ...
  // Fetch ePrivacy regulation by slug
  ...
  // Applicability for this company
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Cookie / ePrivacy toolkit ── */}

      {/* Cookie Banner Guidance */}
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] p-6">
        <CookieBannerGuidance />
      </div>

      {/* Cookie Policy Template Generator */}
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] p-6">
        <CookiePolicyGenerator companyName={company.name} />
      </div>
    </div>
  );
}
